...
public class Main {
    ...
    /*public static void medicSkill() {
            ...
            if (heroesHealth[i] < 100 && heroesHealth[i] > 0) {
                ...
                {Random random = new Random();
                    ...
                    if (i == 3 || heroesHealth[i]>0 || heroesHealth[i]>100) {
                        ...
                        heroesHealth[i] = heroesHealth[i] + h*coeff;
                    }
                } else {

                    int h = 25;

                    if (i == 3 || heroesHealth[i]<1 || heroesHealth[i]>100) {
                        heroesHealth[i] = heroesHealth[i] + 0;
                    } else {
                        heroesHealth[i] = heroesHealth[i] + h;
                    }
     ...
     public static void medicSkillic() {
        for (int i = 0; i < heroesHealth.length; i++) {
            if (i == 3) {
                continue;
            }
   ...
   /* public static void golemSkill(){
        ...
        int howAliveHerro=0;
        for(int i=0; i< heroesHealth.length;i++){
            if(!(i==4)&&heroesHealth[4]<1&&heroesHealth[i]<1){
                howAliveHerro++;
                heroesHealth[i]=heroesHealth[i]+dmg;
            }else {
                heroesHealth[4]=heroesHealth[4]-dmg*howAliveHerro;
                System.out.printf("Голем получил урон:" + dmg*howAliveHerro);
                break;
            }
        }
    }*/
   public static void golemSkill() {
       ...
       System.out.println("Голем получил урон:" + dmg * howAliveHerro);
   }
   ...
   public static void thour(){
       ...
       for (int i = 0; i < heroesHealth.length; i++) {
           if (heroesHealth[7] > 0) {
               if (stn){
               bossDamage=0;
               System.out.println("тор застанил"+stn);
               break;}
           }else{
               bossDamage=50;
               break;
